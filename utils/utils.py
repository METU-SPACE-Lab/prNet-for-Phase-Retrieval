...
import torch
from torch.fft import fft2, fftn, ifft2, ifftn
...
def fft2d(tensor: torch.Tensor) -> torch.Tensor:
    if tensor.ndim not in [3, 4]:
        raise ValueError(
            f"Expected dimension of input (B, C, H, W) or (C, H, W),\
                           but get {tensor.ndim}-D input."
        )
    return fftn(tensor, dim=(-2, -1), norm="ortho") / 2.0
...
def ifft2d(tensor: torch.Tensor) -> torch.Tensor:
    ...
    return ifftn(tensor, dim=(-2, -1), norm="ortho") * 2.0


def split_amplitude_phase(
...
def flip_to_minimize_loss(
    tensor: torch.Tensor, gt_tensor: torch.Tensor
) -> Tuple[torch.Tensor, torch.Tensor]:
    tensor_flipped = tensor.flip(dims=(-1, -2))

    loss = F.mse_loss(tensor, gt_tensor, reduction="none").mean((-1, -2))
    loss_flipped = F.mse_loss(tensor_flipped, gt_tensor, reduction="none").mean(
        (-1, -2)
    )

    condition = loss_flipped < loss
    mask = condition.unsqueeze(-1).unsqueeze(-1).expand_as(tensor)

    return torch.where(mask, tensor_flipped, tensor), torch.where(
        condition, loss_flipped, loss
    )
